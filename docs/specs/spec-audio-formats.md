...
| `mp3` and `flac` are the muxer-enforced `stream_limit` exemption `docs/design/degradation-ladder.md` names by example: both map audio *blindly* yet declare `stream_limit=1`, which is legitimate only because the row above already established that a surplus fails the cheap attempt outright rather than reaching the success-side check | Issue #40 formalized this as the mirror of the `mov`/attachment force-failure exemption (issue #39), and `tests/test_profiles.py` pins it with an `mp3`-shaped stand-in (`MP3_SHAPED`) ahead of this phase's own profile landing | 2026-08-26 |
