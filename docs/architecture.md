...
2. **Degradation.** The attempt exits non-zero, so *now* `ffmpegtool.probe_streams`
   ...
   fallback encoder, which may itself carry a source-dependent option: a value
   the rule declares and the engine appends only when a further probed source
   property — independent of whichever one routed the stream to this branch —
   says the fallback would otherwise need it (`webm`'s video rule is the only
   one that declares one today, an
   ...
   Every sacrifice becomes a note on the attempt. The last rung is the full re-encode the profile declares; a
